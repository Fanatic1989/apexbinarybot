...
def _parallel_scan(markets):
    ...
    if total < 2:
        # Allow solo trade ONLY if signal is 3/3 HIGH (score=2 pts)
        if total == 1 and signals_found[0][0] == 2:
            _, best_market, best_signal, _ = signals_found[0]
            log.info(f"[BOT] Solo HIGH signal — {best_market} "
                     f"{best_signal.get('direction')} — trading alone")
